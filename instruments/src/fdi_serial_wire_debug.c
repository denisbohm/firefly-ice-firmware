...
bool call_apollo_helper_function(fdi_serial_wire_t *serial_wire, uint32_t address, uint32_t *parameters, uint32_t parameter_count, fdi_serial_wire_debug_error_t *error) {
...
*
*******************************************/
bool flash_mass_erase_from_sram(fdi_serial_wire_t *serial_wire, uint32_t instance, fdi_serial_wire_debug_error_t *error) {
    uint32_t parameters[] = {instance, PROGRAM_KEY};
    return call_apollo_helper_function(serial_wire, 0x08000069, parameters, sizeof(parameters) / sizeof(parameters[0]), error);
}
...
bool flash_program_main_from_sram(fdi_serial_wire_t *serial_wire, uint32_t address, uint8_t *data, uint32_t length, fdi_serial_wire_debug_error_t *error) {
    ...
    return call_apollo_helper_function(serial_wire, 0x0800005d, parameters, sizeof(parameters) / sizeof(parameters[0]), error);
}
...
*
*******************************************/
bool flash_program_info_area_from_sram(fdi_serial_wire_t *serial_wire, uint32_t offset, uint8_t *data, uint32_t length, fdi_serial_wire_debug_error_t *error) {
    if (!fdi_serial_wire_debug_write_data(serial_wire, WRITE_BUFFER_START, data, length, error)) {
        return false;
    }
    uint32_t parameters[] = {offset, 0, length / 4, PROGRAM_KEY};
    return call_apollo_helper_function(serial_wire, 0x08000061, parameters, sizeof(parameters) / sizeof(parameters[0]), error);
}
...
*
*******************************************/
bool flash_info_erase_from_sram(fdi_serial_wire_t *serial_wire, fdi_serial_wire_debug_error_t *error) {
    uint32_t parameters[] = {0, PROGRAM_KEY};
    return call_apollo_helper_function(serial_wire, 0x08000085, parameters, sizeof(parameters) / sizeof(parameters[0]), error);
}
...
bool fdi_serial_wire_debug_test(fdi_serial_wire_t *serial_wire) {
    ...
    result = flash_mass_erase_from_sram(serial_wire, 0 /* instance */, &error);
    address = 0x00002000;
    result = fdi_serial_wire_debug_read_data(serial_wire, address, verify, sizeof(verify), &error);
    result = flash_program_main_from_sram(serial_wire, address, data, sizeof(data), &error);
    result = fdi_serial_wire_debug_read_data(serial_wire, address, verify, sizeof(verify), &error);
