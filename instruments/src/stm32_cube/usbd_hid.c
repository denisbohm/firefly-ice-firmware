...
__ALIGN_BEGIN static uint8_t HID_ReportDesc[HID_REPORT_DESC_SIZE] __ALIGN_END =
{
    ...
    0x95, USB_OTG_MAX_EP0_SIZE,         //   REPORT_COUNT (64)
...
static uint8_t USBD_HID_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  if (pdev->dev_speed == USBD_SPEED_HIGH)
  {
    ...
    pdev->ep_out[HID_EPIN_ADDR & 0xFU].bInterval = HID_HS_BINTERVAL;
  }
  else   /* LOW and FULL-speed endpoints */
  {
    pdev->ep_in[HID_EPIN_ADDR & 0xFU].bInterval = HID_FS_BINTERVAL;
    pdev->ep_out[HID_EPIN_ADDR & 0xFU].bInterval = HID_FS_BINTERVAL;
  }
  ...
  extern uint8_t fdi_usb_hid_rx_buffer[64];
