...
void fd_storage_buffer_unit_tests(void) {
    ...
    fd_storage_buffer_add_time_series_s_float16(&storage_buffer, time, interval, 1.0);
    time += interval;
    fd_storage_buffer_add_time_series_s_float16(&storage_buffer, time, interval, 2.0);
    fd_log_assert(fd_storage_used_page_count() == 0);
    fd_storage_buffer_flush(&storage_buffer);
    fd_log_assert(fd_storage_used_page_count() == 1);
    time += interval;
    fd_storage_buffer_add_time_series_s_float16(&storage_buffer, time, interval, 3.0);
    fd_log_assert(fd_storage_used_page_count() == 1);
    fd_storage_buffer_flush(&storage_buffer);
