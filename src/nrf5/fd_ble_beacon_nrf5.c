...
static uint8_t * m_get_adv_packet(void)
{
    ...
    // Constructing header
    adv_pdu[offset]    = 2;    // Advertisement type ADV_NONCONN_IND
...
void m_handle_start(void)
{
    ...
    NRF_PPI->CH[PPI_CH].TEP    = (uint32_t)(&NRF_RADIO->TASKS_TXEN);
    NRF_PPI->CH[PPI_CH].EEP    = (uint32_t)(&NRF_TIMER0->EVENTS_COMPARE[0]);
...
void fd_ble_beacon_init(fd_ble_beacon_init_t * p_init)
{
